...
build-tag-vectors.py と assign-tags.py の双方が使う。タグ側はタグ名を、
記事側は記事から選んだ5語を連結した文字列を、同じ設定で埋め込む。
設定が食い違うと比較が成立しない。

モデルの選定理由と API の詳細は docs/architectural-decision.md の ADR-0004、
タグ名をベクターに用いる理由は ADR-0006 を参照。
"""
...
def load_vectors():
    """tag-vectors.json を読み、モデル・次元・算出方式の一致を確認する。"""
    ...
    if data.get("vector_source") != VECTOR_SOURCE:
        raise SystemExit(
            f"Error: tag-vectors.json のベクターは {data.get('vector_source') or 'centroid（旧方式）'}"
            f" で算出されています。現在の方式は {VECTOR_SOURCE} です。"
            " scripts/build-tag-vectors.py で再算出してください")
    return data
