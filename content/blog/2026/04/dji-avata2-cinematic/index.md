...
*（2026-05-29更新：Post Productionの手順を見直しました）*
...
Frame Rateは撮影時と同じ60FPSに設定します。60FPSで撮影した素材のフレームレートをタイムライン上で24FPSや30FPSに下げても、各フレームが持つ1/120秒のブラー量は変わらないため、シネマティックなモーションブラーが自然に増えるわけではありません。シネマティックな質感は、フレームレートを下げるのではなく、後述のRSMBでブラーを付加して作ります。

DJI Air 3Sなど他のカメラの素材と混在させる場合も、撮影時にあらかじめ60FPSへ揃えておくのが原則です。フレームレートの異なる素材を1つのタイムラインに混在させると、リタイミングによるカクつきの原因になります。
...
撮影は4K 60FPS・シャッタースピード1/120秒（180度ルール）で行っています。この設定は60FPSでは適正な露光ですが、24FPS・1/48秒で撮影した映画のような大きめのモーションブラーは持たず、動きがシャープでビデオ的な印象になります。

フレームレートを24FPSや30FPSに下げても、各フレームが持つ1/120秒のブラー量は変わらないため、モーションブラーが自然に増えるわけではありません。そこでフレームレートは60FPSのまま維持し、RSMB（ReelSmart Motion Blur）で1/48秒相当のブラーを付加して映画的な質感を作ります。RSMBはフレーム間のモーションベクトルを解析してブラーを合成するOFXプラグインです。

RSMBはEditページでも適用できますが、LUTの後段でグレーディング済みの映像に対して処理するため、Colorページで適用します。前述のカラーグレーディングでSkyGrades LUTを適用したNode 2の後ろにシリアルNode 3を追加し、そのノードにRSMBを適用します。

ブラー量はRSMBのBlur Amountで調整します。Blur Amountは1.0がシャッター角360度（フレーム間隔いっぱいのブラー、60FPSでは1/60秒相当）、0.5が180度（1/120秒相当）に対応します。目標の1/48秒は「(1/48秒) ÷ (1/60秒) = 1.25」となるため、Blur Amountを1.25に設定すると1/48秒相当のブラーになります。

ただし素材にはすでに1/120秒の実ブラーが含まれているため、まずBlur Amountを1.0〜1.25の範囲で適用し、映像を見ながら好みのブラー量に追い込んでください。
